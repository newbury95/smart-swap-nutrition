  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-soft-green">
      ...
      <section className="container mx-auto px-4 pt-20 pb-32">
        ...
        >
          <span className="px-4 py-2 bg-soft-purple/30 rounded-full text-sm font-medium inline-block mb-6">
          ...
          <button className="bg-black text-white px-8 py-4 rounded-full font-medium hover:bg-gray-800 transition-colors duration-300 flex items-center gap-2 mx-auto">
            Start Your Journey
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, index) => (
              <motion.div
                key={feature.title}
                className="p-6 rounded-2xl bg-white border border-gray-100 hover:shadow-lg transition-shadow duration-300"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <feature.icon className="w-12 h-12 text-soft-purple mb-4" />
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {conditions.map((condition, index) => (
              <motion.div
                key={condition.name}
                className="p-6 rounded-2xl bg-white/80 backdrop-blur-sm border border-gray-100"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <span className="inline-block px-3 py-1 bg-soft-purple/20 rounded-full text-sm font-medium mb-4">
                  {condition.category}
                </span>
